...
<input type="button" name="range" id="range" value="Search" class="btn btn-primary float-right form-control"/>
 				 ...
 				 <?php
			        ...
			        $view_arts_query="select * from appointments where meeting_partner like '%$email%' or email = '$email' and meeting_partner != 'single' and meeting_partner != '' and meeting_status = 'Started' order by modified_at desc limit 1";
										        ...
										        while($row=mysqli_fetch_array($run22))
										        {
										        	 ...
										        	 $permission = $row['permission'];
										        	   ...
										        	   echo '<p>' . $i .'. '. $row['emp_name'] . '
										        	    ...
										        	    <button type="button" name="accept" class="btn btn-sm bg-success-light"><i class="fas fa-check"></i> '.$permission.'</button>
